...
1. 下載並解壓縮 `Open-Wallpaper-Engine-Mac-Universal.zip`。
2. 把 **Open Wallpaper Engine.app** 拖到「應用程式」。
3. 第一次開啟時，在 App 上按右鍵並選擇「打開」。
4. 若 macOS 仍阻擋，前往「系統設定 → 隱私權與安全性」，在安全性區段按「仍要打開」。

這是未使用 Apple Developer ID 簽署、也尚未經 Apple 公證的開源 App，因此會出現安全提醒。不需要安裝 Xcode，也不需要停用整台 Mac 的 Gatekeeper。
